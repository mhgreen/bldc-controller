...
async def mdc100_manager(shutdown_manager, receive_command):
    logger.debug(f'mdc100_manager starting')
        ...
        async with SerialStream(
            ...
            async with receive_command:
                async for command in receive_command:
                    logger.debug(
                        f'mdc100_manager got command: {command.decode().rstrip()}')
                    ...
                    with trio.move_on_after(0.1) as cancel_scope:
                        ...
                        output_list = [element.decode(
                            'utf8') for element in output_list]
    ...
    if shutdown_manager.shutting_down:
        logger.debug(f'mdc100_manager stopped')
        return
...
async def main(mdc100_port):
    logger.info('bldc controller started')
    ...
    async with open_nurseries() as nurseries:
        ...
        send_command, receive_command = trio.open_memory_channel(1)
        async with send_command, receive_command:
            ...
            nursery.start_soon(
                mdc100_manager, shutdown_manager, receive_command.clone())
    logger.info('bldc controller stopped')
